...
signals:
   void frameReviewLink(GitServerPlugin::PullRequest pr, const QMap<int, int> &links);
...
private:
   ...
   void processComments(const GitServerPlugin::Issue &issue);
   QLabel *createHeadline(const QDateTime &dt, const QString &prefix = QString());
   void onReviewsReceived();
   QLayout *createBubbleForComment(const GitServerPlugin::Comment &comment);
   QLayout *createBubbleForReview(const GitServerPlugin::Review &review);
   QVector<QLayout *> createBubbleForCodeReview(int reviewId, QVector<GitServerPlugin::CodeReview> &comments);
