   ...
   , mSettingsKey(settingsKey)
{
   ...
   {
      mPreRequisites = 0;
      mUserId = QSettings().value(QString("%1/%2-userId").arg(mSettingsKey, mRepoName), "").toString();
      mRepoId = QSettings().value(QString("%1/%2-repoId").arg(mSettingsKey, mRepoName), "").toString();
...
void GitLabRestApi::onUserInfoReceived()
{
   ...
   if (!tmpDoc.isEmpty())
   {
      ...
      if (!list.isEmpty())
      {
         ...
         QSettings().setValue(QString("%1/%2-userId").arg(mSettingsKey, mRepoName), mUserId);
   ...
   if (!tmpDoc.isEmpty())
   {
      ...
      for (const auto &projObj : projectsObj)
      {
         ...
         if (labelMap.value("path").toString() == mRepoName)
         {
            ...
            QSettings().setValue(QString("%1/%2-repoId").arg(mSettingsKey, mRepoName), mRepoId);
            --mPreRequisites;
