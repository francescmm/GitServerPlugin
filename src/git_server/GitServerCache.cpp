...
bool GitServerCache::init(GitServerPlugin::ConfigData data)
{
   mInit = true;
...
void GitServerCache::onCodeReviewsReceived(int number, const QVector<GitServerPlugin::CodeReview> &codeReviews)
{
...
void GitServerCache::onCommentReviewsReceived(int number, const QMap<int, GitServerPlugin::Review> &commentReviews)
{
   if (mPullRequests.contains(number))
   {
...
void GitServerCache::onCommitsReceived(int number, const QVector<GitServerPlugin::Commit> &commits, int currentPage,
                                       int lastPage)
{
   if (mPullRequests.contains(number))
   {
...
PullRequest GitServerCache::getPullRequest(const QString &sha) const
{
   const auto iter = std::find_if(mPullRequests.constBegin(), mPullRequests.constEnd(),
                                  [sha](const GitServerPlugin::PullRequest &pr) { return pr.state.sha == sha; });
...
GitServerPlugin::Platform GitServerCache::getPlatform() const
{
