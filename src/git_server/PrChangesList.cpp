...
PrChangesList::PrChangesList(const QSharedPointer<GitBase> &git, const QString &style, QWidget *parent)
   : QFrame(parent)
   , mGit(git)
   , mStyle(style)
{
...
void PrChangesList::loadData(const GitServerPlugin::PullRequest &prInfo)
{
   GitExecResult ret;
   bool showDiff = true;
   QString head;
   ...
   if (ret.success)
   {
      ...
      if (!changes.isEmpty())
      {
         ...
         {
            const auto changeListItem = new PrChangeListItem(change, mStyle);
